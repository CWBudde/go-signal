...
func eventDoc(evt signal.Event) any {
	...
	case *signal.IdentityChanged:
		return identityChangedDoc{
			eventHead: head(typeIdentityChanged), Recipient: recipient(evt.Recipient),
...
func (p *Printer) identityChangedLine(evt *signal.IdentityChanged) string {
	...
	return prefix + who(evt.Recipient) + ": [safety number changed; sending to them is blocked until you run: " +
