...
func NewGroupJSON(group signal.Group) GroupJSON {
	...
	for _, member := range group.Members {
		out.Members = append(out.Members, GroupMemberJSON{
			recipientJSON:    recipient(member.Recipient),
	...
	for _, pending := range group.Pending {
		out.Pending = append(out.Pending, PendingMemberJSON{
			recipientJSON: recipient(pending.Recipient),
			Role:          pending.Role.String(),
			AddedBy:       recipient(pending.AddedBy),
	...
	for _, requesting := range group.Requesting {
		out.Requesting = append(out.Requesting, RequestingMemberJSON{
			recipientJSON: recipient(requesting.Recipient),
...
func NewLeftGroupJSON(res signal.LeaveResult) LeftGroupJSON {
	...
	for _, rcpt := range res.Promoted {
		out.Promoted = append(out.Promoted, recipient(rcpt))
	}
...
func (p *Printer) Groups(groups []signal.Group) error {
	if p.format == JSON {
		...
		for _, group := range groups {
			doc.Groups = append(doc.Groups, NewGroupJSON(group))
		}
...
func (p *Printer) Group(group signal.Group) error {
	if p.format == JSON {
		return p.writeJSON(groupDoc{Version: SchemaVersion, Group: NewGroupJSON(group)})
	}
	...
	for _, member := range group.Members {
		fmt.Fprintf(table, "  %s\t%s\n", who(member.Recipient), member.Role)
	}
	...
	if len(group.Pending) > 0 {
		...
		for _, pending := range group.Pending {
			fmt.Fprintf(table, "  %s\t%s\tinvited by %s\t%s\n",
				who(pending.Recipient), pending.Role, who(pending.AddedBy), p.dateTime(pending.InvitedAt))
		}
	}
	...
	if len(group.Requesting) > 0 {
		...
		for _, requesting := range group.Requesting {
			fmt.Fprintf(table, "  %s\t%s\n", who(requesting.Recipient), p.dateTime(requesting.RequestedAt))
		}
	}
...
func (p *Printer) LeftGroup(res signal.LeaveResult) error {
	if p.format == JSON {
		return p.writeJSON(leftDoc{Version: SchemaVersion, Left: NewLeftGroupJSON(res)})
	}
